...
open class LensControlWidget @JvmOverloads constructor(
    ...
    private var first_len_btn = findViewById<Button>(R.id.first_len_btn)
    private var second_len_btn = findViewById<Button>(R.id.second_len_btn)
    ...
    override fun reactToModelChanges() {
        addReaction(widgetModel.properCameraVideoStreamSourceRangeProcessor.toFlowable()
            .observeOn(ui()).subscribe(
                {
                updateBtnView()
                },
                {
                    error -> Log.d("test","properCameraVideoStreamSourceRangeProcessor error ",error)
                }
            ))
        addReaction(widgetModel.cameraVideoStreamSourceProcessor.toFlowable()
            ...
            updateBtnView()
                ...
                {
                    error -> Log.d("test","cameraVideoStreamSourceProcessor error", error)
                }
            ))
    ...
    private fun dealLensBtnClicked(source: CameraVideoStreamSourceType) {
        ...
        addDisposable(widgetModel.setCameraVideoStreamSource(source).observeOn(ui()).subscribe(
            ...
            {
                error -> Log.d("test","setCameraVideoStreamSource",error)
            }
        ))
    }
