...
//        layout_camera_mode_p.isSelected = true
    ...
    override fun reactToModelChanges() {
        addReaction(widgetModel.exposureModeProcessor.toFlowable()
            ...
            .subscribe(
                { exposureModeProcessor ->
                  updateExposureMode(exposureModeProcessor)
                },
                {
                  error -> Log.d("test","exposureModeProcessor error",error)
                }
            ))
        addReaction(widgetModel.exposureModeRangeProcessor.toFlowable()
            .observeOn(SchedulerProvider.ui())
            .subscribe(
                {
                    exposureModeRangeProcessor ->
                    updateExposureModeRange(exposureModeRangeProcessor)
                },
                {
                    error -> Log.d("test","exposureModeRangeProcessor error", error)
                }
            ))
    }
