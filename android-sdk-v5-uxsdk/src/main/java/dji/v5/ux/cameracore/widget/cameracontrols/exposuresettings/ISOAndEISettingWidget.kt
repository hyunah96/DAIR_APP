...
open class ISOAndEISettingWidget @JvmOverloads constructor(
    ...
    override fun reactToModelChanges() {
        ...
        // ISO part
        addReaction(widgetModel.exposureModeProcessor.toFlowable()
            .observeOn(SchedulerProvider.ui())
            .subscribe(
                { exposureMode ->
                    onExposureModeUpdated(exposureMode)
                    updateISOEnableStatus()
                },
                { error ->
                    Log.d("test", "reactToModelChanges", error)
                }
            ))
        addReaction(widgetModel.ISOProcessor.toFlowable()
            .observeOn(SchedulerProvider.ui())
            .subscribe(
            { exposureMode ->
                onISOUpdated(exposureMode)
            },
            { error ->
                Log.d("test","reactToModelChanges",error)
            }
        ))
        addReaction(widgetModel.ISORangeProcessor.toFlowable()
            .observeOn(SchedulerProvider.ui())
            .subscribe(
                { exposureMode ->
                    updateISORangeValue(exposureMode.toTypedArray())
                    updateISOEnableStatus()
                    updateISORangeUI()
                },
                {
                    error -> Log.d("test","reactToModelChanges",error)
                }
            ))
        addReaction(widgetModel.exposureSettingsProcessor.toFlowable()
            .observeOn(SchedulerProvider.ui())
            .subscribe(
                { exposureSettings ->
                val exposureParameters = exposureSettings
        ...
        // EI part
        addReaction(widgetModel.eiValueProcessor.toFlowable()
            .observeOn(SchedulerProvider.ui())
            .subscribe(
                {
                ...
                {
                    error -> Log.d("test","eiValueProcessor error",error)
                }
            ))
        addReaction(widgetModel.eiValueRangeProcessor.toFlowable()
            .observeOn(SchedulerProvider.ui())
            .subscribe(
                {
                ...
                {
                    error -> Log.d("test","eiValueRangeProcessor error", error)
                }
            ))
        addReaction(widgetModel.eiRecommendedValueProcessor.toFlowable()
            .observeOn(SchedulerProvider.ui())
            .subscribe(
                {
                ...
                {
                    error -> Log.d("test","eiRecommendedValueProcessor",error)
                }
            ))

        // mode
        addReaction(widgetModel.exposureSensitivityModeProcessor.toFlowable()
            .observeOn(SchedulerProvider.ui())
            .subscribe(
                {
                ...
                {
                    error -> Log.d("test","exposureSensitivityModeProcessor error" , error)
                }
            ))
        addReaction(widgetModel.cameraModeProcessor.toFlowable()
            .observeOn(SchedulerProvider.ui())
            .subscribe(
                {
                ...
                {
                    error -> Log.d("test","cameraModeProcessor error", error)
                }
            ))
        addReaction(widgetModel.flatCameraModeProcessor.toFlowable()
            .observeOn(SchedulerProvider.ui())
            .subscribe(
                {
                ...
                {
                    error -> Log.d("test","flatCameraModeProcessor", error)
                }
            ))
    }
